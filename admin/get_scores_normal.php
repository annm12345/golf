...
if (isset($_GET['id'])) {
    $event_id = $_GET['id'];
    $res = mysqli_query($con, "SELECT `score`.*, `member`.`name`, `member`.`image`,`member`.`hc` FROM `score` JOIN `member` ON `score`.`m_id` = `member`.`id` WHERE `event_id`='$event_id'");
    $scoreData = array();
    ...
    while ($row = mysqli_fetch_assoc($res)) {
        ...
        $row['sumOfscore']=$sumOfscore;
        $HC = 36 - $sumOfMarks; // Assuming $marks is the sum of marks
        $NET = $sumOfscore - ($row['hc'] + $sumOfPar);
